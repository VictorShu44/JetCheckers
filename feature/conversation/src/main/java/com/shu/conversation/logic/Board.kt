...
class CheckersGame {
    ...
    fun handleCellClick(row: Int, col: Int) {
        ...
        if (currentSelected == null) {
            ...
            if (tryMove(srcRow, srcCol, row, col)) {
                ...
                Log.d(
                    "mov",
                    "$count mustCapture $mustCapture isCapture ${row - srcRow}  src $srcRow $srcCol can capture $row $col"
                )
                ...
                if (gameState.value.currentPlayer == Player.WHITE) {
                    Log.d(
                        "mov",
                        "$count Ход не удался, возможно, игрок хочет выбрать другую шашку "
                    )
                    if (clickedPiece != null && clickedPiece.owner == gameState.value.currentPlayer) {
    ...
    fun moveBlack() {
        moveAI = false
        val move = mediumAI.findBestMove(board.cells, Player.BLACK)
        moveRight = move

        move?.let { move ->
            Log.d(
                "mov",
                "$count ход moveAI r ${move.from.row}, ${move.from.col}"
            )
            handleCellClick(move.from.row, move.from.col)
        }
        val allCatch = move.intermediatePositions
        if (allCatch.isNotEmpty()) {
            var t = ""
            allCatch.forEach { move ->
                //  handleCellClick(move.row, move.col)
                t = t + "[${move.row}-${move.col}] "
            }
            Log.d(
                "mov",
                t
